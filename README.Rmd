...
For model data, the public data from the proprietary model written by the data
journalists at FiveThirtyEight. For market data, Victoria University of
Wellington, New Zealand provided the detailed price history on their 
[PredictIt exchange][27].

![data logos](plots/predictr.png)

## Reproduce

All public input data has been archived on the [internet archive][28] and can be
accessed through their wayback machine.

Data reading, wrangling, combining, and visualizing is done primarily using `R`
packages from the [`tidyverse`][29] collection. The following packages should
allow you to run the project from start to finish:
...
The `R` scripts in the [`/code`](/code) folder can be run in sequantial order to
reproduce the project findings.

1. Scrape archived data with the `readr` package and [`01_read_inputs.R`](code/01_read_inputs.R)
1. Format data with the `dplyr` package and [`02_format_inputs.R`](code/02_format_inputs.R)
1. Compare predictive methods with the `tidyr` package and [`03_compare_methods.R`](code/03_compare_methods.R)
1. Create visualizations with the `ggplot2` package and [`04_explore_viz.R`](code/04_explore_viz.R)
1. Run statistical tests with the `stats` package and [`05_analyze_stat.R`](code/05_analyze_stat.R)
