...
The goal of these mathematical forcasting models, [according to Silver][21], is
...
The exact code of the FiveThirtyEight model is proprietary, so we can't know
exactly what data is being incorporated in what ways. We do know that the
"classic" version of their model uses three types of quantitative data:

1. **Polling**: District levek polling. [FiveThirtyEight rates pollsters][09] to
...
The goal of forcasting models is "
...
[Monte Carlo simulation][10]. By estimating the probability distribution in two
candidate's shares of the vote, such simulation can be run tens of thousands of
times. From these simulations, the percentage of one candidate winning is equal
to their probability of winning on election day.

According to FiveThirtyEight, the classic model correctly predicted predicted
**96.7%** of races in their training data of past elections. FiveThirtyEight
points out that the _vast_ majority of those races are blowouts and very easy to
predict.
...
input_fnames <- list.files(path = "./input", full.names = TRUE)
input_files <- lapply(input_fnames, read_csv)
input_onames <- 
  list.files(path = "./input", full.names = FALSE) %>% 
  str_remove(".csv")

for (i in 1:length(input_files)) {
  assign(input_onames[i], input_files[[i]])
}
```

```{r read_output, echo=FALSE, message=FALSE, warning=FALSE}
output_fnames <- list.files(path = "./output", full.names = TRUE)
output_files <- lapply(output_fnames, read_csv)
output_onames <- 
  list.files(path = "./output", full.names = FALSE) %>% 
  str_remove(".csv")

for (i in 1:length(output_files)) {
  assign(output_onames[i], output_files[[i]])
}
```
...
For model data, the `race` variable is created by combinding the `state` and
...
exist for the most contentious and popular raves, are more uniform it its
distribution of probabilities.

```{r prob_dist, echo=FALSE, message=FALSE, warning=FALSE}
left_join(x = model,
          y = market,
