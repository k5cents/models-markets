...
model_history <- bind_rows(model_district, model_seat)
```

```{r model_stats}
avg_daily <- model_history %>% 
...
predictions from `r min(model_history$forecastdate)` to
`r max(model_history$forecastdate)`.

For each observation, there are `r length(model_history)` variables recorded:
