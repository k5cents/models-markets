...
model_history <- 
  read_csv("./data/model_history.csv",
           col_types = cols()) %>% 
  select(-last, -name, -incumbent, chamber)
...
joined$party[which(joined$name == "Kevin de Leon")] <- "R"
joined[which(joined$name == "Chris Pappas" & joined$price == 0.01), ] <- NA

joined %>% 
  select(-name, -chamber, -incumbent, -voteshare, -mid, -cid) %>% 
  print()
```
...
joined_tidy <- 
  ...
  select(date, code, party, tool, prob) %>%
  arrange(date, code)
...
```{r}
IN99 <- filter(joined_tidy, code == "IN-99" & party == "D")
ggplot(IN99) +
  geom_line(aes(x = date, 
                y = prob, 
                color = tool),
            size = 2) +
  labs(title = "Joe Donnelly Victory Over Time",
       y = "Probability",
       x = "Date") +
  scale_x_date() + 
  geom_vline(aes(xintercept = IN99$date[which(IN99$prob == 0.560)][1])) +
  geom_hline(aes(yintercept = 0.50))
```

## Probability Over Time
```{r}
MA99 <- filter(joined_tidy, code == "MA-99" & party == "D")
ggplot(MA99) +
  geom_line(aes(x = date, 
                y = prob, 
                color = tool),
            size = 2) +
  labs(title = "Elizabeth Warren Victory Over Time",
       y = "Probability",
       x = "Date") +
  scale_x_date() + 
  geom_hline(aes(yintercept = 0.50))
...
```{r aug10 label, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r nov05 label, echo=FALSE, message=FALSE, warning=FALSE}
...
## Conclusion
```{r}
spread(accuracy, key = tool, value = accuracy) %>% 
  slice(c(1, 44, 88))
```
