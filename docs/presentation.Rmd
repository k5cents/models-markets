...
read_csv("./input/house_district_forecast.csv") %>% 
  select(forecastdate,
         state,
         district,
         party,
  ...
  kable(digits = 2,
        col.names = c("Date",
                      ...
                      "Prob",
                      "Share"),
        caption = "10 of 299,760 observations with 7 of 12 variables")
```
...
readLines(con = file("./input/DailyMarketData.csv",
  ...
  read_delim(delim = "|",
    ...
    select(MarketSymbol, Date, ClosePrice, Volume) %>% 
  sample_n(10) %>%
  arrange(Date) %>% 
  kable(digits = 3,
        col.names = c("Ticker", "Date", "Price", "Volume"),
        caption = "10 of 44,711 observations with 6 of 11 variables")
```
